# 🌙 Alura Eclipse

> **Automatização inteligente para cursos da Alura via Sala do Futuro**

[![Discord](https://img.shields.io/badge/Discord-Comunidade-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.com/invite/hdY6SDUe8J)
[![Website](https://img.shields.io/badge/Site_Oficial-crimsonzerohub.xyz-FF6B6B?style=for-the-badge&logo=google-chrome&logoColor=white)](https://crimsonzerohub.xyz/)
[![Status](https://img.shields.io/badge/Status-Online-00D26A?style=for-the-badge)](https://crimsonzerohub.xyz/)

---

## 🚀 O que é o Alura Eclipse?

**Alura Eclipse** é um sistema de automação desenvolvido pela **Eclipse Lunar** que **completa automaticamente cursos e atividades** da plataforma Alura através da integração com a Sala do Futuro. Projetado para estudantes que buscam otimizar seu tempo de aprendizado.

---

## ⚙️ Como Funciona

### 🎯 Processo Automatizado
1. **Autenticação**: Login via credenciais SEDUCSP ou cookies da Alura
2. **Busca Inteligente**: Identifica todos os cursos disponíveis
3. **Processamento de Tarefas**: Completa vídeos, exercícios e atividades
4. **Finalização Automática**: Envia avaliação e conclui o curso

---

## 🌐 Como Usar

### 1️⃣ Acesso via Site 
Acesse [crimsonzerohub.xyz/alura](https://crimsonzerohub.xyz/alura) e use a interface web completa.

### 2️⃣ Métodos de Login

**Opção A: Login SEDUCSP**
- Insira seu **RA** e **Senha** da Sala do Futuro
- Sistema obtém acesso automaticamente à Alura

**Opção B: Gmail + Senha**
- Insira seu **E-mail** e **Senha** da sua conta Google
- Sistema faz login direto na Alura

**Opção C: Cookies Diretos (Usuários Google)**
- Tutorial no [Disocrd Server](https://discord.com/invite/hdY6SDUe8J)
- Login mais rápido, mas pode expirar
- Ideal para uso ocasional ou quem entra via Google
- 
### 3️⃣ Selecione Cursos
- Visualize todos os cursos disponíveis
- Veja progresso atual de cada curso
- Marque os cursos que deseja completar

### 4️⃣ Configure Automação
- **Tempo entre tarefas**: Defina delay (padrão: 10 segundos)
- Quanto maior o delay, mais "humano" parece
- Recomendado: 10-30 segundos

### 5️⃣ Inicie o Processo
- Clique em **"Processar Curso"**
- Acompanhe progresso em tempo real

### 6️⃣ Finalização (se necessário)
- Cursos 100% completos podem ser finalizados manualmente
- Sistema envia avaliação automaticamente
- Certificado liberado na Alura

---

## ⚙️ Características Técnicas

### 🎓 Tipos de Atividades Suportadas

| Tipo | Descrição | Automação |
|------|-----------|-----------|
| **VIDEO** | Vídeos de aula | ✅ Marca como assistido |
| **SINGLE_CHOICE** | Questões múltipla escolha | ✅ responde corretamente |
| **SORT_BLOCKS** | Ordenar blocos de código | ✅ responde corretamente |
| **LINK_SUBMIT** | Envio de links/projetos | ✅ Envia link genérico |
| **VISIT_ONLY** | Textos/explicações | ✅ Marca como visitado |

---

## 🔐 Segurança & Privacidade

- ✅ **Tokens criptografados** - Armazenados com segurança no servidor
- ✅ **Delays realistas** - Simula comportamento humano
- ✅ **Session management** - Cookies gerenciados corretamente

---

## ⚠️ Avisos Importantes

### 📌 Sobre Progresso
- Sistema marca tarefas como completas
- Progresso é atualizado em tempo real
- Alguns cursos podem exigir finalização manual

### ⏱️ Recomendações de Uso
- Use delays de 10-30 segundos (mais seguro)
- Não processe muitos cursos simultaneamente
- Verifique progresso após conclusão
- Finalize cursos 100% manualmente se necessário

---

## 🤝 Comunidade & Suporte

### 💬 Discord Oficial
Junte-se à nossa comunidade para:
- Suporte técnico especializado
- Atualizações em primeira mão
- Discussões sobre automação
- Reportar bugs e sugestões
- Compartilhar experiências

**[📥 Entrar no Discord](https://discord.com/invite/hdY6SDUe8J)**

### 🌐 Site Oficial
Visite [crimsonzerohub.xyz](https://crimsonzerohub.xyz/) para:
- Interface web completa
- Documentação técnica atualizada
- Changelog de versões
- FAQ detalhado
- Tutoriais em vídeo

---

## 🚫 Anti-Pirataria & Versões Oficiais

**⚠️ ATENÇÃO: Cuidado com golpistas!**

### 🔒 Código Fechado
Este projeto **NÃO é open source**. O código-fonte não está disponível publicamente.

### 💎 Versões Disponíveis

**🆓 Versão FREE (Gratuita)**
- Automação completa de cursos
- Todos os tipos de atividades
- Processamento em background
- Suporte básico no Discord
- **100% gratuito para sempre**

**⭐ Versão PREMIUM (Paga)**
- Suporte técnico dedicado 24/7
- Acesso antecipado a novos recursos
- Configurações avançadas
- **APENAS disponível através do Discord Oficial**

### 🚨 ALERTA DE FRAUDE

Se alguém **fora do Discord oficial** tentou vender acesso ao **Alura Eclipse**:

1. 🚨 **Você está sendo enganado** - Não é revendedor autorizado
2. 📢 **Denuncie no Discord** - Reporte o golpista para a equipe
3. 💸 **Não pague** - Versões premium só pelo Discord oficial

**Não existem:**
- ❌ Revendedores autorizados fora do Discord
- ❌ Vendedores em Telegram/WhatsApp/Instagram
- ❌ Versões "cracked", "hackeadas" ou "modificadas"
- ❌ Licenças vendidas por terceiros
- ❌ "Métodos secretos" pagos

**Canais oficiais:**
- ✅ **Apenas pelo [Discord Oficial](https://discord.com/invite/hdY6SDUe8J)**
- ✅ Negociação direta com administradores verificados
- ✅ Suporte oficial da equipe Eclipse Lunar

**Acesso gratuito:**
- ✅ [Discord Oficial](https://discord.com/invite/hdY6SDUe8J)
- ✅ [Site Oficial](https://crimsonzerohub.xyz/)

---

## 🙏 Créditos

Desenvolvido por **Eclipse Lunar**

**Fundador:** CrimsonStrauss  
**Servidor:** Eclipse Lunar  
**Comunidade:** [Discord](https://discord.com/invite/hdY6SDUe8J)

---

## 📞 Contato

- 💬 Discord: [discord.com/invite/hdY6SDUe8J](https://discord.com/invite/hdY6SDUe8J)
- 🌐 Website: [crimsonzerohub.xyz](https://crimsonzerohub.xyz/)
- 👤 Desenvolvedor: **@CrimsonStrauss** (Discord)

---

<div align="center">

### 🌙 Eclipse Lunar
**Automação Inteligente de Cursos Alura**

[![Discord](https://img.shields.io/badge/Discord-Junte--se-5865F2?style=flat-square&logo=discord&logoColor=white)](https://discord.com/invite/hdY6SDUe8J)
[![Website](https://img.shields.io/badge/Website-Visite-FF6B6B?style=flat-square&logo=google-chrome&logoColor=white)](https://crimsonzerohub.xyz/)

*Feito com 🌙 por estudantes, para estudantes*

</div>

---
